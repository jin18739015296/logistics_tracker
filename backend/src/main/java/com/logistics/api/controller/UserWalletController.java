...
public class UserWalletController {
    ...
    public ResponseEntity<Result<String>> recharge(
        ...
        try {
            ...
            return ResponseEntity.badRequest().body(Result.error(10001, "金额格式错误"));
        }

        if (amount.compareTo(BigDecimal.ZERO) <= 0) {
            return ResponseEntity.badRequest().body(Result.error(10001, "充值金额必须大于0"));
        }

        // 模拟充值：直接增加余额（实际项目中这里应对接支付网关）
    ...
    public ResponseEntity<Result<List<WalletTransaction>>> getTransactions(
            ...
            Principal principal) {
        Long userId = userService.getUserByUsername(principal.getName()).getId();
        List<WalletTransaction> transactions = walletService.getTransactionsWithPagination(userId, type, page, size);
